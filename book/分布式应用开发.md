...
    分布式服务顾名思义服务是分散部署在不同的机器上的，一个服务可能负责几个功能，是一种面向SOA（Service-Oriented Architecture）架构的，服务之间也是通过rpc来交互或者是RestfulAPI来交互。

    分布式系统是现代计算机科学中的一个重要领域，它涉及到多个计算机节点之间的协同工作，以实现更高的性能、可用性和可扩展性。
...
a、脆弱，只要有一个环节出问题，整个系统就会崩溃。

b、随着互联网用户迅速发展，传统单体架构技术难以支撑海量用户的需求，无法应对高并发请求。

c、维护成本高，每个地方修改都会影响到整体。无法应对复杂业务带来的各种挑战。
...
a、**负载均衡**：通过负载均衡器将请求分发到各个节点上，使各节点的负载均衡。常见的负载均衡算法有轮询、加权轮询、最少连接等。

b、**消息队列**：通过消息队列实现节点间的异步通信，提高系统的并发处理能力。常用的消息队列工具有RabbitMQ、Kafka等。
c、**服务化架构**：将系统划分为多个服务，每个服务独立部署、独立维护，通过API网关进行服务间的调用和通信。

d、**缓存Redis**：常用数据放到缓存层，实现快速读取，减小计算压力以及IO存取压力。
e、**数据库集群**：通过将数据分散到多个数据库节点上，实现数据的水平和垂直拆分，提高数据库的读写性能和并发处理能力。

## 4、分布式架构总体架构图

<img title="" src="./images/no1_fenbushijiagoutu.png" alt="" width="1083">

## 5、分布式架构的优势和挑战

**优势**：
a、**高性能**：通过负载均衡和并行处理，提高系统的处理能力和响应速度。
b、**高可用性**：通过节点间的冗余备份和故障恢复机制，确保系统的可用性。
c、**可伸缩性**：根据负载变化自动扩展或缩减节点数量，实现系统的可伸缩性。
d、**灵活性**：可以快速部署新服务和功能，降低开发、维护成本。
**挑战**：
a、**通信开销**：节点间的通信和协作可能带来较大的网络开销。
b、**分布式一致性**：在分布式系统中实现数据一致性是一个挑战。需要采用合适的分布式一致性协议，如CAP理论、Paxos、Raft等。
c、**系统复杂性**：分布式系统中的节点间依赖关系复杂，可能导致系统故障传播和性能瓶颈。
d、**容错性**：某个节点发生故障时，如何保证整个系统的稳定性和可用性是一个挑战。
...
a、随着业务复杂性的增加，开发变得困难，增加了研发成本抑制了研发效率的提升。
b、系统的运维成本很高，对某个功能的修改，都需要对整个系统进行构建、测试、部署。
c、技术栈单一，不能够很好地解决业务的多样化需求。
d、隔离性差，一个模块出现问题，可能导致整个系统崩溃。
e、可伸缩性差，对于部分业务的高并发，不能按需扩展。

    为了解决上述问题，微服务架构应运而生。简单来说，微服务就是将一个单体应用拆分成若干个小型的服务，协同完成系统功能的一种架构模式，在系统架构层面进行解耦合，将一个复杂问题拆分成若干个简单问题。下面是**采用微服务架构带来的优势**：

a、服务承载的业务逻辑内聚，易于开发与维护。
b、单个服务启动快。
c、技术栈灵活，独立的服务可以选用不同的技术栈。
d、独立的按需扩展，满足高并发与大流量。
...
a、用户管理模块：用户注册、登录、个人信息管理、订单查看、跟踪等功能。

b、后台商品管理模块：包括商品的添加、编辑、审核、上下架、库存管理、品牌管理等。

c、安全校验模块：短信验证码、邮箱验证码等安全验证。

d、订单模块：下订单、计算订单总价格、快递费等。

e、支付模块：集成多种支付模式。

f、减库存、订单状态模块：订单状态管理（已发货、已收货、售后中、已关闭等），减库存

g、营销与促销模块：优惠券、满减、限时打折、发放红包等活动设置和管理。

h、聊天与订单状态推送模块：实现客户售后服务、订单状态推送等功能。

i、热销商品、排行榜模块：实现热销商品、推荐商品、商品排行榜等功能。

j、第三方接口模块：物流、图像识别、身份认证。
...
Go 语言既有Python语言的灵活语法和开发效率，又有接近C语言的运行性能。
...
<img title="" src="./images/no1_gmp01.png" alt="" width="1205">
...
a、protobuf协议是跨语言跨平台的序列化协议。
b、protobuf本身并不是和GRPC绑定的。它也可以被用于非RPC场景，如存储等
...
a、下载proto win版本。下载地址：[https://github.com/protocolbuffers/protobuf/releases]([Releases · protocolbuffers/protobuf · GitHub](https://github.com/protocolbuffers/protobuf/releases))

b、解压，放到任意目录下（例如：C:\protobuf）
...
如果安装后有错。可能有多个go版本。设置系统变量 path ，添加路径 C:\Users\Administrator\go\bin
...
a、从 A 账户取 100 元（数据表字段减100）。
b、存入 100 元至 B 账户（数据表字段加100）。
这两步要么一起完成，要么一起不完成，如果只完成第一步，第二步失败，钱会莫名其妙少了 100 元。

因此，需要事务来协同完成，以确保数据的一致性。两步都操作成功，那么此次操作执行完成。只要有一步不成功，执行失败。

在单体服务中，事务的一致性，可以依靠数据库的事务来完成。但在实际应用中，单体应用比较少。大多有用户量的企业，都是微服务化架构。各个服务被部署在不同的机器上。这个时候就需要分布式事务，来确保数据的一致性。
